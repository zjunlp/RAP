...
def main(
    ...
    print(os.getcwd())
    label_list = json.load(open("./outputs/data/relations/" + base_data + "_relations_list.json", "r"))
    # 将新关系加入词表
    vocabulary = tokenizer.get_vocab().keys()
    print("vocab len before add:", len(vocabulary))
    for label in label_list:
        if label not in vocabulary:
            tokenizer.add_tokens(label)
    # 保存关系的map字典
    print("vocab len after add:", len(tokenizer.get_vocab().keys()))
    label_dict = {key: tokenizer.get_vocab()[key] for key in label_list}
    json.dump(label_dict, open("./outputs/data/relations/" + base_data + "_relations_dict.json", "w"))
