...
class InputFeatures(object):
    ...
    def __init__(self,
        ...
        self.origin_attention_mask = origin_attention_mask
...
def convert(example, max_text_len, tokenizer, rel2idx, data_sign, ex_params):
    ...
    origin_attention_mask = None
    # 获取初始的attention_mask
    if "[SEP]" in text_tokens:
        idx = text_tokens.index("[SEP]")
        origin_attention_mask = attention_mask[:idx]
        ori_pad_len = max_text_len - len(origin_attention_mask)
        origin_attention_mask += [0] * ori_pad_len
    ...
    if data_sign == 'train':
        ...
        for rel, en_ll in example.rel2ens.items():
            ...
            sub_feats.append(InputFeatures(
                ...
                relation=rel,
                rel_tag=rel_tag,
                origin_attention_mask=origin_attention_mask
            ))
